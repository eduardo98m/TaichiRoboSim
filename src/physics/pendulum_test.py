...
from collision import PlaneCollider, SphereCollider, Collider, BoxCollider
from collision import PLANE, SPHERE, BOX
...
def main():
    ...
    #sqrt2 = np.sqrt(2.0)
    sphere_2 = RigidBody(mass = 1.0,
                        inertia = ti.Matrix(np.eye(3,3, dtype = np.float32)),
                        position = ti.Vector([0.0, 0., -0.5], dt=ti.f32), 
                        orientation = ti.Vector([1.0, 0.0, 0.0, 0.0], dt = ti.f32),
                        velocity = ti.Vector([0.1, -0.001, 0.0], dt = ti.f32),
                        angular_velocity = ti.Vector([0.0, 0.0, 0.0], dt = ti.f32),
                        fixed = False,
                        material = ceramic,
                        collider = sphere_collider)
    
    sphere_3 = RigidBody(mass = 10.0,
                        inertia = ti.Matrix(np.eye(3,3, dtype = np.float32)),
                        position = ti.Vector([0.0, 0.0, -1.0], dt=ti.f32), 
    ...
    world.create_hinge_joint_constraint(
        ...
        relative_orientation = ti.Vector([np.pi, 0.0, 0.0], dt = ti.f32),
        r_1 = ti.Vector([ 0.0, 0.0, 0.0], dt = ti.f32),
        r_2 = ti.Vector([-1.0, 0.0, 0.0], dt = ti.f32),
        damping = 0.0,
        compliance = 0.00
    )
    ...
    #     relative_orientation = ti.Vector([0.0, 0.0, 1.0], dt = ti.f32),
    #     r_1 = ti.Vector([0.0, 0.0, 0.0], dt = ti.f32),
    #     r_2 = ti.Vector([1.0, .0, 0.0], dt = ti.f32),
    #     damping = 7.5,
    #     compliance = 0.0
    # )
