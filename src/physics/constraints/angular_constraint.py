...
def compute_angular_constraint(
    ...
    if theta <= EPSILON:
        ...
        new_lagrange_mult = lagrange_mult + angular_constraint_lagrange_multiplier_update(theta, w_1, w_2, lagrange_mult, h, compliance)
    ...
    if not body_1.fixed:
        new_orientation_1 = tm.normalize(body_1.orientation + quaternion.rotate_vector(body_1.orientation, 0.5 * body_1.dynamic_inv_interia @ impulse))
        
    
    if not body_2.fixed:
        new_orientation_2 = tm.normalize(body_2.orientation - quaternion.rotate_vector(body_2.orientation, 0.5 * body_2.dynamic_inv_interia @ impulse))
